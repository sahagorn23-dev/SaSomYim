  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Reload page to re-run Server Component check (verifyStaffSession)
    ...
    <div className="min-h-screen bg-cream flex flex-col items-center justify-center p-5 font-ibm">
      <div className="w-full max-w-sm bg-card rounded-[24px] shadow-lg p-8">
        <div className="text-center mb-8">
          <h1 className="font-kanit font-bold text-2xl text-ink">Staff Access</h1>
          <p className="text-sm text-muted mt-2">กรอก PIN เพื่อเข้าสู่ระบบจัดการแต้ม</p>
        </div>
